...
function genericBgHash() {
  for (let i = 0; i < ((window.innerWidth / (16 * 0.7)) * (window.innerHeight / (16 * 1))); i++ ) {
    ...
    addSymbol.appendChild(text);
    bgHash.appendChild(addSymbol);
  }
};
...
function generateHash(e) {
  let hash = keyWord.value;
  hash = hash.length + hash + hash.length;
  hash = MD5(hash);
  let A = hash.substring(0, 3);
  let protoA = hash.substring(4, 8);
  A = symbolize(A, protoA);
  let B = hash.substring(8, 11);
  let protoB = hash.substring(12, 16);
  B = symbolize(B, protoB);
  let C = hash.substring(16, 19);
  let protoC = hash.substring(20, 24);
  C = symbolize(C, protoC);
  let D = hash.substring(24, 27);
  let protoD = hash.substring(28, 31);
  D = symbolize(D, protoD);
  hash = D + A + B + C;
  let upperCase = "";
  for (let i = 0; i < hash.length; i++) {
    upperCase += i % 2 == 0 ? hash.charAt(i).toUpperCase() : hash.charAt(i);
  };
  hash = upperCase;
  password.value = hash;
...
genPass.onclick = generateHash;
/****************************/

/****************************/
function symbolize(substring, prototype) {
  let sum = 0;
  sum = 0;
  let numstring = "";
  numstring = prototype.replace(/\D/g, "");
